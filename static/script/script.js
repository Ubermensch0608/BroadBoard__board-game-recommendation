...
function btn_next() {
    ...
    for (var i = 0; i < active_elements.length; i++) {
        if (k_str == '') k_str = active_elements[i].innerHTML;
        else k_str = k_str + ',' + active_elements[i].innerHTML;
    }
    if (k_str == '') {
    ...
    if (txt != '') {
        ...
        ref.forEach(function(arg) {
            if (arg.name.indexOf(txt) > -1) {
                $('#autoMaker').append($('<div class="hint">').text(arg.name).attr({ key: arg.key }));
                $('#autoMaker').show();
            }
        });
    } else {
                    ...
                    if (target_game == games[i]['title'].replace(' ', '')) {
                        ...
                        for (n = 0; n < like.length; n++) {
                            if (like[n] == login_info.id) {
                                ...
                                ).innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" fill="white" width="20" height="20" viewBox="0 0 24 24"><path d="M12 9.229c.234-1.12 1.547-6.229 5.382-6.229 2.22 0 4.618 1.551 4.618 5.003 0 3.907-3.627 8.47-10 12.629-6.373-4.159-10-8.722-10-12.629 0-3.484 2.369-5.005 4.577-5.005 3.923 0 5.145 5.126 5.423 6.231zm-12-1.226c0 4.068 3.06 9.481 12 14.997 8.94-5.516 12-10.929 12-14.997 0-7.962-9.648-9.028-12-3.737-2.338-5.262-12-4.27-12 3.737z"/></svg>
                        ...
                        if (n == like.length) {
                            ...
                            document.querySelector(
                                'button.recommend-btn'
                            ).innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" fill="white" width="20" height="20" viewBox="0 0 24 24"><path d="M12 4.435c-1.989-5.399-12-4.597-12 3.568 0 4.068 3.06 9.481 12 14.997 8.94-5.516 12-10.929 12-14.997 0-8.118-10-8.999-12-3.568z"/></svg>
                                                                                                 <b class="recommend-number">${cur_likes}</b>`;
                            isRecommend = 'true';
                        }
                        return;
...
// 회원가입 기능
let member_list = [{ id: 'id', pwd: 'password' }];
